...
template<typename T>
bool GTNetwork::bfs(T s, T t, vector<T>& parent, unordered_map<T, unordered_map<T, int>>& capacity) {
...
int GTNetwork::edmondsKarp(const T& source, const T& sink) {
    ...
    vector<T> parent(this->graph.size());
    int maxFlow = 0;
...
template<typename T>
vector<T> GTNetwork::getMAXFlowPath(const T& source, const T& sink, vector<T>& parent) {
    vector<T> path;
    for (auto v = sink; v != source; v = parent[v]) {
        path.push_back(v);
    }
